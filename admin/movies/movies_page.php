...
if (isset($_POST['updateMovie'])) {
    ...
    $posterPath = $_POST['oldPoster']; // default to existing poster path

    // Handle new poster upload (optional on update)
    if (!empty($_FILES['Poster']['tmp_name'])) {
        ...
        if ($_FILES['Poster']['size'] > $maxSizeBytes) {
            echo "<p class='text-red-600 text-center mt-4'>New poster file too large (max 5MB).</p>";
                ...
                if (!in_array($imageType, [IMAGETYPE_JPEG, IMAGETYPE_PNG], true)) {
                    ...
                    if (!move_uploaded_file($tmpPath, $absolutePath)) {
                        ...
                        try {
                            ...
                            if (!empty($posterPath)) {
                                $oldAbsolute = __DIR__ . '/../../' . $posterPath;
...
<div class="flex justify-center">
    <a href="add_movies_page.php">
        <button
            class="border border-amber-400 text-amber-400 m-6 px-8 py-3 rounded-full font-bold hover:bg-amber-400 hover:text-slate-800 transition text-[30px]">
            ADD MOVIE
        </button>
    </a>
</div>
        ...
        <?php foreach ($movies as $row): ?>
            <tr class="border-b hover:bg-gray-50">
                ...
                <td class="p-3 text-slate-800 flex gap-2">
                    <button onclick="toggleEdit(<?= $row['MovieID'] ?>)"
                            class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded">Edit
                    </button>
            ...
            <tr id="editRow<?= $row['MovieID'] ?>" class="hidden bg-amber-50">
                <td colspan="6" class="p-4">
                    <form method="POST" enctype="multipart/form-data"
                        ...
                        <input type="text" name="Titel" value="<?= politi($row['Titel']) ?>"
                               class="border p-2 rounded w-full text-slate-800" required>
                        <input type="number" name="ageRating" value="<?= $row['ageRating'] ?>"
                               class="border p-2 rounded w-full text-slate-800">
                        <input type="number" name="Duration" value="<?= $row['Duration'] ?>"
                               class="border p-2 rounded w-full text-slate-800">
                        <textarea name="Description"
                                  class="border p-2 rounded w-full col-span-2 text-slate-800"><?= politi($row['Description']) ?></textarea>
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="file" name="Poster" accept="image/*"
                                   class="border p-2 rounded w-full text-slate-800">
                            <?php if (!empty($row['Poster'])): ?>
                                <img src="/<?= politi($row['Poster']) ?>" alt="poster"
                                     class="w-16 mt-2 rounded">
                            <?php endif; ?>
                        </div>

                        <button type="submit" name="updateMovie"
                                class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded col-span-2 w-max">
                            Save Changes
                        </button>
                    </form>
                </td>
