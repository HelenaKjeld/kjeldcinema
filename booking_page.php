          ...
          <div class="grid grid-cols-10 gap-3" id="seats">
            <?php if ($seats): ?>
              ...
              foreach ($grouped as $row => $rowSeats):
                ...
                usort($rowSeats, function ($a, $b) { return $a['SeatNumber'] - $b['SeatNumber']; });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const dkk = (function() {
    try {
      return new Intl.NumberFormat('da-DK', { style: 'currency', currency: 'DKK', currencyDisplay: 'narrowSymbol' });
    } catch(e) {
      // Fallback if Intl is missing (rare)
      return { format: n => `kr ${Number(n).toFixed(2).replace('.', ',')}` };
    }
  })();
