    ...
    <?php foreach ($movies as $movie): ?>
        <div class="bg-slate-800 rounded-xl overflow-hidden mb-12 shadow-lg">
            <div class="flex flex-col md:flex-row">
                <div class="md:w-1/4">
                    <img src="/<?= politi($movie['Poster']) ?>"
                        alt="<?= politi($movie['Titel']) ?>"
                        class="w-full h-full object-cover">
                </div>

                <div class="md:w-3/4 p-6">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                        <h3 class="text-2xl font-bold text-white"><?= politi($movie['Titel']) ?></h3>
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        <span class="text-xs bg-slate-700 px-2 py-1 rounded"><?= politi($movie['ageRating']) ?>+</span>
                        <p>Age</p>
                        <span class="text-xs bg-slate-700 px-2 py-1 rounded"><?= politi($movie['Duration']) ?> min</span>
                    </div>

                    <p class="text-gray-300 mb-6"><?= politi($movie['Description']) ?></p>
